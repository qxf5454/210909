...
class CameraOperation():
    ...
    def __init__(self,obj_cam,st_device_list,n_connect_num=0,b_open_device=False,b_start_grabbing = False,h_thread_handle=None,\
        ...
        self.obj_cam = obj_cam                      #相机句柄 obj
        self.st_device_list = st_device_list        #设备列表 
        self.n_connect_num = n_connect_num          #连接数量 int
        self.b_open_device = b_open_device          #设备打开 bool
        self.b_start_grabbing = b_start_grabbing    #开始抓图 bool
        self.b_thread_closed = b_thread_closed      #线程关闭 bool
        self.st_frame_info = st_frame_info          #帧信息
        self.b_exit = b_exit                        #退出     bool
        self.b_save_bmp = b_save_bmp                #保存bmp  bool
        self.b_save_jpg = b_save_jpg                #保存jpg  bool
        self.buf_save_image = buf_save_image        #存图缓存  buf
        self.h_thread_handle = h_thread_handle      #线程句柄   handle
        self.n_win_gui_id = n_win_gui_id            #窗口界面id?  int
        self.n_save_image_size = n_save_image_size  #存图大小 int
        self.b_thread_closed                        #线程关闭 bool
        self.frame_rate = frame_rate                #帧率        int
        self.exposure_time = exposure_time          #曝光时间    int
        self.gain = gain                            #增益        int
    ...
    def Open_device(self):
        if False == self.b_open_device:
            ...
            if ret != 0:
                ...
                print("create handle fail! ret = "+ self.To_hex_str(ret))
                return ret

            ret = self.obj_cam.MV_CC_OpenDevice(MV_ACCESS_Exclusive, 0)
            if ret != 0:
                print("open device fail! ret = "+ self.To_hex_str(ret))
                return ret
    ...
    def Start_grabbing(self,root,panel):
        if False == self.b_start_grabbing and True == self.b_open_device:
            ...
            if ret != 0:
                print("start grabbing fail! ret = "+ self.To_hex_str(ret))
                return
    ...
    def Stop_grabbing(self):
        if True == self.b_start_grabbing and self.b_open_device == True:
            ...
            if ret != 0:
                print("stop grabbing fail! ret = "+self.To_hex_str(ret))
                return
    ...
    def Close_device(self):
        if True == self.b_open_device:
            ...
            if ret != 0:
                print("close deivce fail! ret = "+self.To_hex_str(ret))
                return
    ...
    def Set_trigger_mode(self,strMode):
        if True == self.b_open_device:
            if "continuous" == strMode: 
                ...
                if ret != 0:
                    print("set triggermode fail! ret = "+self.To_hex_str(ret))
            if "triggermode" == strMode:
                ...
                if ret != 0:
                    print("set triggersource fail! ret = "+self.To_hex_str(ret))
    ...
    def Trigger_once(self,nCommand):
        if True == self.b_open_device:
            if 1 == nCommand: 
                ...
                if ret != 0:
                    print("set triggersoftware fail! ret = "+self.To_hex_str(ret))
    ...
    def Set_parameter(self,frameRate,exposureTime,gain):
        if '' == frameRate or '' == exposureTime or '' == gain:
            print("please type in the text box !")
            return
        if True == self.b_open_device:
    ...
    def Work_thread(self,panel):
        ...
        while True:
            ...
            current_image = cv.fromarray(numArray)
    ...
    def Save_jpg(self,buf_cache):
        ...
        if return_code != 0:
            print("save jpg fail! ret = %s",self.To_hex_str(return_code))
    ...
    def Save_Bmp(self,buf_cache):
        ...
        return_code = self.obj_cam.MV_CC_SaveImageEx2(stParam)            
        if return_code != 0:
            print("save bmp fail! ret = %s ",self.To_hex_str(return_code))
    ...
    def Mono_numpy(self,data,nWidth,nHeight):
        data_ = np.frombuffer(data, count=int(nWidth * nHeight), dtype=np.uint8)
        numArray = data_.reshape(nHeight, nWidth)
    ...
    def Color_numpy(self,data,nWidth,nHeight):
        data_ = np.frombuffer(data, count=int(nWidth*nHeight*3), dtype=np.uint8)
        ...
        numArray=cv.merge(data_r_arr,data_g_arr,data_b_arr)
